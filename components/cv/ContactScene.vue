<template>
  <section id="contact" class="py-24 relative overflow-hidden" ref="sectionRef">
    ...
    <div class="absolute inset-0 flex items-center justify-center pointer-events-none" ref="radarContainerRef">
      <div
        v-for="i in 4"
        :key="i"
        class="radar-ring absolute rounded-full border border-accent/0"
    ...
    <div class="max-w-6xl mx-auto px-6 relative z-10">
      ...
      <div ref="headerRef" class="mb-16">
        <div class="flex items-center gap-3 mb-4">
          ...
          <span class="text-xs font-mono tracking-widest text-accent uppercase opacity-0" ref="eyebrowTextRef">Get in touch</span>
        </div>
        <h2 class="text-4xl md:text-5xl font-bold tracking-tight text-zinc-50 opacity-0" ref="titleTextRef">Contact</h2>
      </div>

      <div class="max-w-2xl">
        <p ref="descRef" class="text-zinc-400 text-base leading-relaxed mb-8 opacity-0">
          Feel free to reach out — whether it's about a project, an opportunity, or just to connect.
        </p>
...
function pulseRing(event: MouseEvent) {
  const target = event.currentTarget as HTMLElement;
  const ring = document.createElement('div');
  ring.className = 'absolute rounded-full border border-accent/40 pointer-events-none';
  ring.style.cssText = 'width: 40px; height: 40px; left: 20px; top: 50%; transform: translateY(-50%);';
